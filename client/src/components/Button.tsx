...
export const SubmitButton: React.FC<ButtonProps> = ({
  ...
  <button
    ...
    className={classNames(
      className,
      "inline-flex justify-center items-center px-4 py-2 text-sm shadow-sm rounded-md text-white",
      loading
        ? "font-semibold bg-indigo-500 hover:bg-indigo-400 transition ease-in-out duration-150 cursor-not-allowed"
        : "border border-transparent bg-indigo-600 font-medium hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
    )}
    {...rest}
  >
    {loading && <LoadingSpin />}
